...
fn main() {
    ...
    let mut tokens = lexer::lex_file_to_tokens(&src_file); 
    if debug {
        tokens.iter().for_each(|t| println!("[INFO token]: {t}"));
    }

    // parse tokens to operations
    let operations = analyser::compile_tokens_to_operations(&mut tokens);
    if debug {
        operations.iter().for_each(|o| println!("[INFO op]: {o}"));
    }
    
    // analyser::crossreference_blocks(&mut operations);
